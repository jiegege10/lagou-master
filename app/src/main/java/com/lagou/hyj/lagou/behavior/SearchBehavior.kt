...
class SearchBehavior:CoordinatorLayout.Behavior<View> {
    ...
    override fun onDependentViewChanged(parent: CoordinatorLayout?, child: View?, dependency: View): Boolean {
        if(child is ViewGroup){
            ...
            Log.d("AAAA","expend:"+expend.toString())
            //箭头展示,动画结束后显示
            val ivArrow = child.getChildAt(0)
            if (mOnListener!=null)
            {
                mOnListener!!.onListener(!expend)
                mOnListener!!.onXListener(currY==-0.5f||expend)
            }
            if(!expend){
    ...
    var mOnListener: OnListener? = null

    interface OnListener {
        fun onListener(force:Boolean)
        fun onXListener(force:Boolean)
    }
    fun setOnListener(mOnListener: OnListener) {
